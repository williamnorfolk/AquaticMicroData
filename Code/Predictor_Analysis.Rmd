...
plaza_arrange_2 <- plaza_time_22 + expand_limits(y = c(0:4)) + ggtitle("Plaza Stream") + xlab("Days Since Rain")
```
...
hos_arrange_2 <- hos_time_22 + expand_limits(y = c(0:4)) + ggtitle("Hospital") + xlab("Days Since Rain")
```
...
vm_arrange_2 <- vm_time_22 + expand_limits(y = c(0:4)) + ggtitle("Vet Med") + xlab("Days Since Rain")
```
```{r}
...
tan_time_2 <- ggplot(mod_tan, aes(x = days_sig_rainfall, y = log_final_ecoli, fill = days_sig_rainfall)) + stat_summary(fun.y = "mean", geom = "bar") + theme_classic() + xlab("Days Since Significant Rainfall") + ylab(mean_ecoli_cfu) + ggtitle(title_vm) + scale_fill_manual(values = c("#004E60", "#00A3AD", "#66435A"))
...
tan_arrange_2 <- tan_time_22 + expand_limits(y = c(0:4)) + ggtitle("Tanyard") + xlab("Days Since Rain")
```
