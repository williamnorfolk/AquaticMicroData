...
```{r}
reduce_vars <- clean_names %>% dplyr::select(sample_id, last_rain, date, ambient_air_high_c, ambient_air_low_c, water_temp, conductivity, ph, final_ecoli, exceeds_epa, confirmed_salmonella, xlt4_salmonella)
...
```{r}
reduce_vars$last_rain <- as.numeric(as.character(reduce_vars$last_rain)) 
